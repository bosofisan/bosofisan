# Hey, I'm Lulu 👋

Platform Engineer specializing in Kubernetes, cloud infrastructure, and ML systems optimization.

I build fault-tolerant, cost-efficient platforms that enable engineering teams to move faster. Currently managing multi-tenant Kubernetes clusters supporting ML workloads at scale.

**Recent wins:**
- 🚀 Improved GPU cluster utilization from 40-55% to 70-85%
- 💰 Reduced infrastructure costs by 40%
- ⚡ Eliminated multi-hour job queues, bringing average wait time to under 5 minutes
- 🛡️ Built fault-tolerant ML infrastructure with 99.9% uptime

## 💻 Tech Stack

**Infrastructure & Orchestration:** Kubernetes, Docker, EKS, Helm  
**Cloud:** AWS (EC2, S3, VPC, RDS, IAM, CloudWatch, ECR)  
**IaC & Automation:** Terraform, GitOps, CI/CD (GitHub Actions)  
**Observability:** Prometheus, Grafana, CloudWatch, distributed tracing  
**Languages:** Python, Go, Bash, SQL  

## 🚀 Featured Projects

| Project | Description | Tech Stack |
|---------|-------------|------------|
| [dockerized-app-ci](https://github.com/bosofisan/dockerized-app-ci) | Containerized Node.js app with CI/CD pipeline | Docker, AWS ECS, GitHub Actions |
| [k8s-deployment-demo](https://github.com/bosofisan/k8s-deployment-demo) | Microservice deployment with Kubernetes | Kubernetes, Helm, YAML |
| [terraform-aws-infra](https://github.com/bosofisan/terraform-aws-infra) | Infrastructure-as-Code for AWS | Terraform, AWS |
| [monitoring-stack](https://github.com/bosofisan/monitoring-stack) | Observability with Prometheus + Grafana | Prometheus, Grafana, K8s |

## 🎓 Certifications

- ☸️ Certified Kubernetes Application Developer (CKAD)
- ☁️ AWS Certified Cloud Operations Administrator

## 📫 Let's Connect

- 💼 [LinkedIn](https://linkedin.com/in/your-linkedin) *(update with your actual LinkedIn)*
- 📧 boluosofisan@gmail.com
- 📍 New York, NY

---

*Building infrastructure that scales, optimizing systems that matter.*
